...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .login-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 2rem; width: 100%; max-width: 360px; }
  ...
  .sidebar-logo { padding: 1.25rem 1rem; font-weight: 700; font-size: 1rem; border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: 0.5rem; }
  .sidebar-logo span { font-size: 1.3rem; }
  .sidebar-user { padding: 0.75rem 1rem; border-bottom: 1px solid var(--border); }
  .sidebar-user .name { font-weight: 600; }
  .sidebar-user .role { color: var(--text-muted); font-size: 0.8rem; }
  nav { flex: 1; padding: 0.5rem 0; }
  nav a { display: flex; align-items: center; gap: 0.625rem; padding: 0.5rem 1rem; color: var(--text-muted); transition: background 0.15s, color 0.15s; }
  ...
  .stat-label { color: var(--text-muted); font-size: 0.8rem; margin-bottom: 0.25rem; }
  .stat-value { font-size: 1.5rem; font-weight: 700; }

  /* Forms */
  .form-group { margin-bottom: 1rem; }
  label { display: block; margin-bottom: 0.35rem; font-weight: 500; }
  input, select, textarea { width: 100%; padding: 0.5rem 0.75rem; background: var(--surface2); border: 1px solid var(--border); border-radius: 6px; color: var(--text); font-size: 14px; font-family: inherit; transition: border-color 0.15s; }
  input:focus, select:focus, textarea:focus { outline: none; border-color: var(--accent); }
  input[type="checkbox"] { width: auto; }

  /* Buttons */
  .btn { display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.45rem 1rem; border-radius: 6px; border: 1px solid transparent; cursor: pointer; font-size: 13px; font-family: inherit; font-weight: 500; transition: background 0.15s, opacity 0.15s; }
  ...
  th { color: var(--text-muted); font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }
  tr:last-child td { border-bottom: none; }
  tr:hover td { background: var(--surface2); }

  /* Badge */
  .badge { display: inline-block; padding: 0.15rem 0.5rem; border-radius: 999px; font-size: 0.75rem; font-weight: 600; }
  .badge-superadmin { background: #3a2060; color: #c084fc; }
  .badge-admin { background: #1a3050; color: var(--accent); }
  .badge-user { background: #1e2535; color: var(--text-muted); }
  ...
  /* Alert */
  .alert { padding: 0.65rem 1rem; border-radius: 6px; margin-bottom: 1rem; }
  .alert-error { background: rgba(224,92,92,0.15); border: 1px solid rgba(224,92,92,0.3); color: #e05c5c; }
  .alert-success { background: rgba(76,175,128,0.15); border: 1px solid rgba(76,175,128,0.3); color: var(--success); }

  /* Tabs */
  .tabs { display: flex; gap: 0.25rem; border-bottom: 1px solid var(--border); margin-bottom: 1.25rem; }
  .tab { padding: 0.5rem 1rem; cursor: pointer; color: var(--text-muted); border-bottom: 2px solid transparent; margin-bottom: -1px; transition: color 0.15s; }
...
<div id="app" class="app hidden">
  <aside class="sidebar">
    <div class="sidebar-logo"><span>🦞</span> OpenClaw</div>
...
(function() {
  ...
  async function loadAgents() {
    ...
    body.innerHTML = '<tr><td colspan="2" class="empty-state"><span class="spin">⟳</span> Loading…</td></tr>';
    const r = await api('GET', '/agents');
    if (!r.ok) { body.innerHTML = '<tr><td colspan="2" class="empty-state">Failed to load agents.</td></tr>'; return; }
    const agents = r.data.agents;
    body.innerHTML = agents.map(a => \`
      <tr>
        <td>\${esc(a.id)}\${a.id === r.data.defaultId ? ' <span class="badge badge-admin">default</span>' : ''}</td>
        <td>\${esc(a.name || '—')}</td>
      </tr>\`).join('');
  }
